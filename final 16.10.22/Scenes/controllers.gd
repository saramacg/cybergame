...
#creating variables needed for controllers to teleport, grip and function correctly
...
#getting teleportation working
func _physics_process(delta):

	if teleport_button_down:
		teleport_raycast.force_raycast_update()
		body_location = teleport_raycast.get_collider()
		if teleport_raycast.is_colliding():
			if teleport_raycast.get_collider() is StaticBody:
				...
				print (body_location)
				get_node("../../" + body_location.name).global_transform.origin = global_transform.origin 
...
func button_released(button_index):
	...
	if button_index == 15:
		...
		if teleport_button_down:
			...
			if teleport_pos and teleport_mesh.visible and body_location.name != "fish_pond":
