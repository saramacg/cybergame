...
func _ready():
	...
	if VR and VR.initialize():
		...
		for i in range(1,4):
			...
			add_child(apple)
			if i == 1:
				get_node("./apple/Sprite3D").set_texture(malwaretextures[0])
				apple.transform.origin = Vector3(13,0, 15)
			elif i ==2:
				get_node("./@apple@2/Sprite3D").set_texture(malwaretextures[1])
				apple.transform.origin = Vector3(13,0, 15)
			else :
				get_node("./@apple@3/Sprite3D").set_texture(malwaretextures[2])
				apple.transform.origin = Vector3(13,0, 15)
